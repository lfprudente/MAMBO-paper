"""
-------------------------------------------------------------------------------- 
**PG-BB! — Barzilai–Borwein Projected Gradient Algorithm for Multiobjective Optimization **
...
**Function signature:**
    ...
    PG-BB!(x, n, m, l, u; epsopt, scaleF, iprint)
...
function PG_BB!(x::Vector{T},
    ...
    header = print_header_BB(n, m, epsopt, scaleF, sF, max_print_obj, iprint)
    ...
    for i in 1:m
        ...
        nfev += 1
        if flag != 1
            return finish_BB(t0, outiter, nfev, ngev, nhev, ZERO, -1, iprint)
        end
    end
    ...
    while true
        ...
        if infoIS != 1
            return finish_BB(t0, outiter, nfev, ngev, nhev, ZERO, -2, iprint)
        end
        ...
        if abstheta <= epsopt
            inform = 1
            return finish_BB(t0, outiter, nfev, ngev, nhev, theta, inform, iprint)
        end
        ...
        if beta_BB != ONE
            infoIS = evalPG!(dBB, lambda, dSD, lambdaSD,
                            n, m, x, beta_BB * JF, l, u)
        ...
        if infoLS < 0
            return finish_BB(t0, outiter, nfev, ngev, nhev, theta,
                          infoLS == -2 ? -1 : -3, iprint)
        end
        ...
        for i in 1:m
            ...
            ngev += 1
            if flag != 1
                return finish_BB(t0, outiter, nfev, ngev, nhev, theta, -1, iprint)
            end
            @views JF[i,:] .= g
        end
    end
...
# Helper
# ------------------------------------------------------------
function print_header_BB(n::Int, m::Int, epsopt::T, scaleF::Bool, sF::Vector{T}, 
    ...
    if iprint
        ...
        @printf("\n    BB : Barzilai–Borwein with Armijo")
...
function finish_BB(t0::T, outiter::Int, nfev::Int, ngev::Int, nhev::Int, 
    theta::T, inform::Int, iprint::Bool)  where {T<:AbstractFloat}
    time_spent = time() - t0
    if iprint
        if inform == 1
            @printf("\nFlag of PG-BB: Solution was found\n")
        elseif inform == 2
            @printf("\nFlag of PG-BB: Maximum of iterations reached\n")
        elseif inform == 3
            @printf("\nFlag of PG-BB: Lack of progress in the functional value, its gradient and the current point\n")
        elseif inform == 4
            @printf("\nFlag of PG-BB: Objective function seems to be unbounded\n")
        elseif inform == -1
            @printf("\nFlag of PG-BB: An error occurred during the evaluation of a function\n")
        elseif inform == -2
            @printf("\nFlag of PG-BB: An error occurred during the subproblem solution\n")
        elseif inform == -3
            @printf("\nFlag of PG-BB: An error occurred during the linesearch\n")
        else
            @printf("\nFlag of PG-BB: Unknown termination code (%d)\n", inform)
        end
